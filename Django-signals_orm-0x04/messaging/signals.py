...
def log_message_old_content(sender, instance, **kwargs):
    ...
    Populates 'edited_by' with the message sender for simplicity given signal limitations.
    """
        ...
        try:
            ...
            if old_instance.content != instance.content:
                MessageHistory.objects.create(
                    ...
                    edited_by=instance.sender # Assuming the message sender is the one editing
                )
                instance.edited = True # Set edited flag when content changes
                print(f"Message {instance.id} old content saved to history by {instance.sender.username}.")
...
def cleanup_user_data(sender, instance, **kwargs):
    ...
    Leverages CASCADE on ForeignKeys for efficiency.
    """
    print(f"User {instance.username} (ID: {instance.id}) deleted. Associated data (messages, notifications) are handled by CASCADE.")
